...
//         c++;  // Increment count for each node
...
//     cout << "Number of nodes: " << count(first) << endl;  // Count and display the number of nodes
//     return 0;
// }
...
int count(struct node *p) {
    if (p == NULL) {   // Base case: if the pointer is NULL, return 0
        ...
        return count(p->next) + 1;  // Recursive call: count the rest of the list and add 1
    }
}
...
int main() {
    int a[] = {3, 5, 7, 10, 15};  // Array to create the linked list from
    create(a, 5);                 // Create the linked list
    int total_nodes = count(first); // Call count and store the result
    cout << "Number of nodes: " << total_nodes << endl;  // Display the count of nodes
    return 0;
}
